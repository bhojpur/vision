...
# Bhojpur Vision version to use.
VISION_VERSION?="v1.0.0"
...
explain:
	@echo "For quick install with typical defaults of both OpenCV and Bhojpur Vision, run 'make install'"
...
# Build a minimal Go app to confirm Bhojpur Vision works.
verify:
	go run ./cmd/version/main.go

# Build a minimal Go app to confirm Bhojpur Vision cuda works.
verify_cuda:
	go run ./cmd/cuda/main.go

# Build a minimal Go app to confirm Bhojpur Vision openvino works.
verify_openvino:
...
releaselog:
	git log --pretty=format:"%s" $(VISION_VERSION)..HEAD
